...
//------------- dvmh_convert_XY() function ------------
__inline__ __device__ void	dvmh_convert_XY(const DvmType x, const DvmType y, const DvmType Rx, const DvmType Ry, const DvmType slash, DvmType &idx)
{
	if(!slash)
	{
		if(Rx == Ry)
...
//--------------------- Kernel for loop on line 16 ---------------------
         ...
         __global__ void   loop_adi_16_cuda_kernel_1_case(double a[], CudaIndexType a0004, CudaIndexType a0003, CudaIndexType a0002, DvmType num_elem_j, DvmType num_elem_k, DvmType base_i, DvmType base_j, DvmType base_k, DvmType step_i, DvmType step_j, DvmType step_k, DvmType type_of_run, DvmType idxs_0, DvmType idxs_1, DvmType idxs_2){
            DvmType id_x, id_y;
            DvmType coords[3];
...
// Loop body
               a[a0004 * i + a0003 * j + a0002 * k] = (a[a0004 * (i - 1) + a0003 * j + a0002 * k] + a[a0004 * (i + 1) + a0003 * j + a0002 * k]) / 2;
            }
         }
...
//--------------------- Kernel for loop on line 24 ---------------------
         ...
         __global__ void   loop_adi_24_cuda_kernel_1_case(double a[], CudaIndexType a0004, CudaIndexType a0003, CudaIndexType a0002, DvmType num_elem_j, DvmType num_elem_k, DvmType base_i, DvmType base_j, DvmType base_k, DvmType step_i, DvmType step_j, DvmType step_k, DvmType type_of_run, DvmType idxs_0, DvmType idxs_1, DvmType idxs_2){
            DvmType id_x, id_y;
            DvmType coords[3];
...
// Loop body
               a[a0004 * i + a0003 * j + a0002 * k] = (a[a0004 * i + a0003 * (j - 1) + a0002 * k] + a[a0004 * i + a0003 * (j + 1) + a0002 * k]) / 2;
            }
         }
...
//--------------------- Kernel for loop on line 33 ---------------------
         ...
         __global__ void   loop_adi_33_cuda_kernel_1_case(double a[], CudaIndexType a0004, CudaIndexType a0003, CudaIndexType a0002, double eps, double eps_grid[], DvmType num_elem_j, DvmType num_elem_k, DvmType base_i, DvmType base_j, DvmType base_k, DvmType step_i, DvmType step_j, DvmType step_k, DvmType type_of_run, DvmType idxs_0, DvmType idxs_1, DvmType idxs_2){
            DvmType id_x, id_y;
            DvmType coords[3];
...
// Loop body
               eps = max(eps, fabs(a[a0004 * i + a0003 * j + a0002 * k] - (a[a0004 * i + a0003 * j + a0002 * (k - 1)] + a[a0004 * i + a0003 * j + a0002 * (k + 1)]) / 2));
               a[a0004 * i + a0003 * j + a0002 * k] = (a[a0004 * i + a0003 * j + a0002 * (k - 1)] + a[a0004 * i + a0003 * j + a0002 * (k + 1)]) / 2;
            }
...
//--------------------- Kernel for loop on line 74 ---------------------
         ...
         __global__ void   loop_adi_74_cuda_kernel(double a[], CudaIndexType a0004, CudaIndexType a0003, CudaIndexType a0002, CudaIndexType blocks[], int nz, int ny, int nx){
...
// Loop body
                     ...
                     {
                        a[a0004 * i + a0003 * j + a0002 * k] = 10. * (i - 1) / (nx - 1) + 10. * (j - 1) / (ny - 1) + 10. * (k - 1) / (nz - 1);
                     }
                     else{
                        a[a0004 * i + a0003 * j + a0002 * k] = 0.000000e+00;
                     }
                  }
               }
            }
...
//    CUDA handler for loop on line 16 
         ...
         void   loop_adi_16_cuda_1_case(DvmhLoopRef *loop_ref, DvmType a[], DvmType type_of_run){
            ...
            DvmType num_y;
            DvmType num_x;
            DvmType a_outTypeOfTfm;
            DvmType a_addressingParams[7];
            DvmType idxs[5];
            DvmType lowI[5], highI[5], idxI[5];

// Autotransform all arrays
            loop_cuda_autotransform(loop_ref, a);
            DvmType device_num;
...
// Fill device headers
            dvmh_fill_header_ex_(&device_num, a_base, a, d_a, &a_outTypeOfTfm, a_addressingParams);
...
//Start method
            ...
            {
               loop_adi_16_cuda_kernel_1_case<<<blocks, threads, shared_mem, stream>>>((double *)a_base, d_a[3], d_a[2], d_a[1], num_elem_x, num_elem_y, base_i, base_j, base_k, idxI[2], idxI[1], idxI[0], type_of_run, idxs[0], idxs[1], idxs[2]);
            }
         }
...
//Start method
            ...
            for (int tmpV = 0 ; tmpV < highI[2] ; base_i = base_i + idxI[2], tmpV = tmpV + 1)
            {
               loop_adi_24_cuda_kernel_1_case<<<blocks, threads, shared_mem, stream>>>((double *)a_base, d_a[3], d_a[2], d_a[1], num_elem_x, num_elem_y, base_i, base_j, base_k, idxI[2], idxI[1], idxI[0], type_of_run, idxs[0], idxs[1], idxs[2]);
            }
         }
...
//Start method
            ...
            for (int tmpV = 0 ; tmpV < highI[2] ; base_i = base_i + idxI[2], tmpV = tmpV + 1)
            {
               loop_adi_33_cuda_kernel_1_case<<<blocks, threads, shared_mem, stream>>>((double *)a_base, d_a[3], d_a[2], d_a[1], eps, cuda_ptr_0, num_elem_x, num_elem_y, base_i, base_j, base_k, idxI[2], idxI[1], idxI[0], type_of_run, idxs[0], idxs[1], idxs[2]);
            }
            tmpVar = 1;
...
// GPU execution
            ...
            {
               loop_adi_74_cuda_kernel<<<blocks, threads, 0, stream>>>((double *)a_base, d_a[3], d_a[2], d_a[1], blocks_info, *nz, *ny, *nx);
            }
         }
