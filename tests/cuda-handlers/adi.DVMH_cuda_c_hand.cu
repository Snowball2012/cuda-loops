...
// Get device number
    device_num = dvmh_loop_get_device_num_C(loop_ref);

// Get natural bases
    a_base = dvmh_get_natural_base_C(device_num, a);

// Fill device headers
    dvmh_fill_header_C(device_num, a_base, a, d_a, 0);

// Get bounds
    dvmh_loop_fill_bounds_C(loop_ref, lowI, highI, idxI);
...
void   loop_adi_41_cuda(DvmhLoopRef pLoopRef, DvmType a_hdr[]){
    int which_run;
    which_run = loop_get_dependency_mask_(pLoopRef);
    if (which_run == 1 || which_run == 2 || which_run == 4)
    {
        loop_adi_24_cuda_1_case(pLoopRef, a_hdr, which_run);
    }
}
